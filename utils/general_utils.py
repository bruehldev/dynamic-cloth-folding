...
def get_randomized_env(env, randomization_kwargs):
    ...
    return wrappers.DomainRandomizationWrapper(
        ...
        randomize_camera=randomization_kwargs["camera_position_randomization"],
...
def argsparser():
    ...
    parser.add_argument("--max-path-length", default=50, type=int)
    ...
    parser.add_argument("--lookat-position-randomization-radius", default=0.03, type=float)
    ...
    parser.add_argument("--rotation-perturbation-size", default=0.75, type=float)
    parser.add_argument("--position-perturbation-size", default=0.2, type=float)

    # Camera
    parser.add_argument("--camera-type", choices=["up", "side", "front", "all"], default="side")
    parser.add_argument("--camera-config", choices=["small", "large"], default="small")

    # Env
    parser.add_argument("--cloth-size", default=0.2, type=float)

    # Image capture delays
    parser.add_argument("--image-obs-noise-mean", type=float, default=0.5)
    parser.add_argument("--image-obs-noise-std", type=float, default=0.5)

    # Which observations from the robot to consider
    parser.add_argument("--robot-observation", choices=["ee", "ctrl", "none"], default="ctrl")
...
def get_path_collector_kwargs(args):
    path_collector_kwargs = dict(
        ...
        demo_paths=[os.path.join("./data", "demos.csv")],
...
def get_env_kwargs(args, save_folder):
    env_kwargs = dict(
        ...
        model_kwargs_path=os.path.join("./data", "model_params.csv"),
    )
    return env_kwargs
