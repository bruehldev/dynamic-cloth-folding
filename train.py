...
pylog = logging.getLogger(BACKEND + ":" + __name__)
...
def experiment(variant: TrainingConfig):
    ...
    runlog = RunLogger(root=run_log_dir, project=f"dynamic-cloth-folding-{BACKEND}")
    ...
    if BACKEND == "bullet":
        ...
        env_kwargs = dict(variant["pybullet"]["env_kwargs"])
        env_kwargs["logger"] = runlog

        variant["pybullet"]["randomization_kwargs"] = make_bullet_randomization_kwargs()
        print("Variant pybullet:", variant["pybullet"])
        eval_env = ClothEnv(
            **env_kwargs, randomization_kwargs=variant["pybullet"]["randomization_kwargs"]
        )
    ...
    print("Variant experiment:", variant)
...
if __name__ == "__main__":
    ...
    variant: TrainingConfig = general_utils.get_variant(args)
